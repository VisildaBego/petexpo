...
const API_URL = 'https://freetestapi.com/api/v1/cats'
const SEARCH_API = 'https://freetestapi.com/api/v1/cats?search='
...
async function getPets(url) {
    try {
        ...
        if (Array.isArray(data)) {
            showPets(data);
        } else if (data && data.cats && Array.isArray(data.cats)) {
            showPets(data.cats);
        } else {
            console.error('Unexpected data structure:', data);
            main.innerHTML = '<p>No cats found.</p>';
        }
    } catch (error) {
        console.error('Error fetching cats:', error);
        main.innerHTML = '<p>Error fetching cats.</p>';
    }
}
...
form.addEventListener('submit', async (e) => {
    ...
    if (searchTerm) {
        ...
        // If search term is empty, reload all cats
        getPets(API_URL);
    }
});